...
def get_localizer(RoI):
    """
    Find the localizer task name based on RoI
    Args:
...
def load_data(sub_id, phase, index, RoI=None, side=None, display=False, data_dir='../data/neural'):
    """Load beta data and RoI mask
    Args:
        sub_id (str): subject index
        phase (str): train or test
        index (str): odd or even (only for test phase, in which videos are presented twice)
        RoI (str): targeted ROI
        side (str): left or right
        display (bool): whether to display the beta data
    Returns:
        tuple: beta data (numpy.ndarray, x, y, z, video) and RoI mask (numpy.ndarray, x, y, z)
    """
    # Load beta data
...
def visualize_slices(beta_data, mask_data, video_id, sub_id, RoI=None, side=None, z_slices=range(25, 65, 10)):
    ...
    for i, z in enumerate(z_slices):
        ...
        axes[i].set_title(f'Z-Slice {z} Visualization\nvideo {video_id}, {roi_text}, subject {sub_id}')
...
if __name__ == '__main__':
    ...
    visualize_slices(beta_data, mask_data, 10, sub_id, "EVC", side)
    # RoI
    RoI = 'TPJ'
    side = 'r'
    video_id = 10
